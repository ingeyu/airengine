...
bool CIOCPModel::Start()
{
	...
	{
		TRACE(_T("Listen Socket初始化失败！\n"));
		this->_DeInitialize();
		return false;
	}
	else
	{
		TRACE(_T("Listen Socket初始化完毕."));
	}

	TRACE(_T("系统准备就绪，等候连接....\n"));
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	{  
		TRACE(_T("绑定 Listen Socket至完成端口失败！错误代码: %d/n"), WSAGetLastError());  
	...
	if(SOCKET_ERROR == WSAIoctl(
		...
		NULL))  
	{  
		TRACE(_T("WSAIoctl 未能获取AcceptEx函数指针。错误代码: %d\n"), WSAGetLastError()); 
		this->_DeInitialize();
		return false;  
	}  
	...
	if(SOCKET_ERROR == WSAIoctl(
		...
		NULL))  
	{  
		TRACE(_T("WSAIoctl 未能获取GuidGetAcceptExSockAddrs函数指针。错误代码: %d\n"), WSAGetLastError());  
		this->_DeInitialize();
		return false; 
	}  
...
//	ASSERT( INVALID_SOCKET!=m_pListenContext->m_Socket );
	...
	{  
		TRACE(_T("创建用于Accept的Socket失败！错误代码: %d"), WSAGetLastError()); 
		return false;  
	} 
	...
	{  
		...
		{  
			TRACE(_T("投递 AcceptEx 请求失败，错误代码: %d"), WSAGetLastError());  
			return false;  
		}  
	} 
...
bool CIOCPModel::_AssociateWithIOCP( PER_SOCKET_CONTEXT *pContext )
{
	...
	if (NULL == hTemp)
	{
		TRACE(_T("执行CreateIoCompletionPort()出现错误.错误代码：%d"),GetLastError());
		return false;
	}
