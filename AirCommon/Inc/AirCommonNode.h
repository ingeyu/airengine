...
#ifndef COMMONNODE_HEADER_FILE
#define COMMONNODE_HEADER_FILE
		...
		*/
		...
		public:
			...
			*/
			...
			typedef std::map<Air::AString, Node*> ChildNodeMap;
			typedef ChildNodeMap::iterator	ChildNodeIterator;

			/** Listener which gets called back on Node events.
			*/
			class COMMON_EXPORT Listener
			{
			public:
				Listener() {}
				virtual ~Listener() {}
		...
		protected:
			...
			*/
			...
			/** Internal method for creating a new child node - must be overridden per subclass. */
			virtual Node* createChildImpl(const AString& name) = 0;
			...
			mutable Matrix mCachedTransform;
		...
		public:
			...
			*/
			...
			/** Moves the node along arbitrary axes.
				...
				relativeTo The space which this transform is relative to.
			*/
			virtual void translate(const Matrix& axes, const Vector3& move, TransformSpace relativeTo = TS_PARENT);
			/** Moves the node along arbitrary axes.
				@remarks
				This method translates the node by a vector which is relative to
				a custom set of axes.
				...
				@param
					relativeTo The space which this transform is relative to.
			*/
			virtual void translate(const Matrix& axes, Real x, Real y, Real z, TransformSpace relativeTo = TS_PARENT);

			/** Rotate the node around the Z-axis.
			*/
			virtual void roll(const Real& angle, TransformSpace relativeTo = TS_LOCAL);

			/** Rotate the node around the X-axis.
			*/
			virtual void pitch(const Real& angle, TransformSpace relativeTo = TS_LOCAL);

			/** Rotate the node around the Y-axis.
			*/
			virtual void yaw(const Real& angle, TransformSpace relativeTo = TS_LOCAL);

			/** Rotate the node around an arbitrary axis.
			*/
			virtual void rotate(const Vector3& axis, const Real& angle, TransformSpace relativeTo = TS_LOCAL);
			...
			*/
			virtual Node* createChild(
				const Vector3& translate = Vector3(0,0,0), 
			...
			@remarks
				...
				@param
					rotate Initial rotation relative to parent
			*/
			virtual Node* createChild(const AString& name, const Vector3& translate = Vector3(0,0,0), const Quaternion& rotate = Quaternion());
			...
			*/
			virtual Node* getChild(const AString& name) const;
			...
			*/
			...
			@remarks
				...
				this parent, potentially to be reattached elsewhere.
			*/
			virtual Node* removeChild(const AString& name);
			...
			*/
			virtual Quaternion & _getDerivedOrientation(void) const;

			/** Gets the position of the node as derived from all parents.
			*/
			virtual Vector3 & _getDerivedPosition(void)const;

			/** Gets the scaling factor of the node as derived from all parents.
			*/
			virtual Vector3 & _getDerivedScale(void)const;
			...
			*/
			virtual Matrix& _getFullTransform(void) const;
