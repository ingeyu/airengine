...
HRESULT CDXUTXFileMesh::Create( LPDIRECT3DDEVICE9 pd3dDevice, LPCWSTR strFilename )
{
    ...
    m_dwNumVertices = m_pMesh->GetNumVertices();//顶点数	
    m_dwNumFaces = m_pMesh->GetNumFaces();//面数 三角形数
    m_dwBytesPerVertex = m_pMesh->GetNumBytesPerVertex();//顶点字节长
    m_pMesh->GetIndexBuffer( &m_pIB );//索引缓冲
    m_pMesh->GetVertexBuffer( &m_pVB );//顶点缓冲
