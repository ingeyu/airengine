...
void CDXUTListBox::Render( float fElapsedTime )
{
    ...
    if( m_Items.GetSize() > 0 )
    {
        ...
        rc.bottom = rc.top + m_pDialog->GetFont(pElement->iFont)->nHeight;//>GetManager()->GetFontNode( pElement->iFont )->nHeight;

        // Update the line height formation
        m_nTextHeight = 21;//rc.bottom - rc.top;
        ...
        for( int i = m_ScrollBar.GetTrackPos(); i < ( int )m_Items.GetSize(); ++i )
        {
            ...
            if( bSelectedStyle )
            {
                rcSel.top = rc.top; rcSel.bottom = rc.top+21;//rc.bottom;
