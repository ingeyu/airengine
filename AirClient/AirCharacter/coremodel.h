...
public:
  ...
  //add	by	infernalhell	20100818	以下几个函数增加用来 支持从资源包加载
  int loadCoreAnimation(void*	pData, const std::string& strAnimationName);
  int loadCoreMesh(void*	pData, const std::string& strMeshName);
  bool loadCoreSkeleton(void*	pData);
