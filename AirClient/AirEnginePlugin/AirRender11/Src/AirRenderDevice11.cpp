			...
			void Device11::DrawOpt( const DrawOption& opt )
			{
				...
				switch(opt.m_DrawFuncType){
					...
					case	Draw::FUNC_TYPE_DIP		:{
						m_pContext->DrawIndexed(opt.m_uiFaceCount*3,opt.m_uiStartIndex,opt.m_uiBaseVertexIndex);
						break;}
					case	Draw::FUNC_TYPE_DA		:{
						m_pContext->DrawAuto();
						break;}
					case	Draw::FUNC_TYPE_DIP_I	:{
						m_pContext->DrawIndexedInstanced(opt.m_uiFaceCount*3,opt.m_uiInstanceCount,opt.m_uiStartIndex,opt.m_uiBaseVertexIndex,0);
						break;}
