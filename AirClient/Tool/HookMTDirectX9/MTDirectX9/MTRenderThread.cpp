...
void	Hook_IDirect3DDevice9(IDirect3DDevice9*	pThis){
		...
		MessageBoxA(NULL,"PAUSE",NULL,NULL);
...
void	MT_RenderThread::RenderFrame(){
		...
		switch(func){
			...
			case enCF_Texture_LockRect:{
				...
				if(pRect!=NULL){
					pRect		=	&param->rData;
					uiHeight	=	pRect->bottom	-	pRect->top;
				}else{
					uiHeight	=	desc.Height;
				}
				...
				if(SUCCEEDED(hr)){
					U8*	pSrc	=	pBuffer;
					U8*	pDst	=	(U8*)rLock.pBits;
					if(param->oldPitch==rLock.Pitch){
						memcpy(rLock.pBits,pBuffer,param->dwLockSize);
					}else{
						U32	uiCopySize	=	min(param->oldPitch,rLock.Pitch);
						for(U32 i=0;i<uiHeight;i++){
							memcpy(pDst,pSrc,uiCopySize);
							pSrc+=param->oldPitch;
							pDst+=rLock.Pitch;
			...
			case enCF_Texture_UnlockRect:{
				...
				hr	=	ST_Texture_UnLockRect(param->pTex,param->Level);
										 }break;
