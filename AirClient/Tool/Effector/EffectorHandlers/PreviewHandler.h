...
#include <afxext.h>
#include "..\Effector\EffectorDoc.h"
#include "..\Effector\EffectorView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CEffectorView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CEffectorDoc, (CObject*) m_pDocument->GetContainer());
