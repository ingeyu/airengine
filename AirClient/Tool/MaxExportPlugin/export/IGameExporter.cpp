...
{
	buf = splitPath;
	buf = buf + "\\" + fileName + "_" + node->GetName() + ".ame";
...
int	IGameExporter::DoExport(const TCHAR *name,ExpInterface *ei,Interface *i, BOOL suppressPrompts, DWORD options)
{
	...
	Air::SetFullPath(name);
	Air::SetPath(path);
	
	Air::Export((options & SCENE_EXPORT_SELECTED) ? true : false);
#else
	HRESULT hr;
